...
export default function Ledger() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--card)' }}>
            ...
            <div style={{ marginTop: 6, fontSize: 13, color: 'var(--muted)' }}>
              {editPhone ? (
                <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  />
                  <button className="btn btn-primary" style={{ padding: '6px 10px' }} onClick={savePhone}>Save</button>
                  <button className="btn btn-ghost" style={{ padding: '6px 10px' }} onClick={() => setEditPhone(false)}>✕</button>
                </span>
              ) : (
                <span>
                  ...
                  <button
                    className="no-print"
                    onClick={() => { setPhoneVal(selectedCustomer.phone || ''); setEditPhone(true) }}
                    style={{ border: 'none', background: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}
                  >
                    Edit
                  </button>
                </span>
      ...
      {/* Add form */}
      {showAdd && (
        <form onSubmit={handleAdd} className="no-print" style={{
          padding: 14, background: 'rgba(24,95,165,0.05)', borderBottom: '1px solid var(--border)'
        }}>
          <div style={{
            display: 'grid',
          ...
          <div style={{ marginTop: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button type="button" className="btn btn-ghost" onClick={() => { setShowAdd(false); setAddForm(emptyTx) }}>
              Cancel
            </button>
          </div>
        </form>
                ...
                <tr style={{ background: 'var(--bg)', borderBottom: '1px solid var(--border)' }}>
                  {['Date', 'Bill No', 'Amount', 'Received', 'Rec. Date', 'Balance', ''].map((h) => (
                    <th key={h} style={{
                      ...
                      fontSize: 11, color: 'var(--muted)', fontWeight: 700, whiteSpace: 'nowrap'
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {txs.map((tx) => {
                  ...
                  return (
                    <tr key={tx.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '11px 14px', whiteSpace: 'nowrap' }}>{formatDate(tx.receivedDate)}</td>
                      <td style={{
                      ...
                      <td className="no-print" style={{ padding: '11px 14px', textAlign: 'right' }}>
                        <button
                          ...
                          style={{ border: 'none', background: 'none', color: 'var(--danger)', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}
                        >
                          Del
                        </button>
                      </td>
